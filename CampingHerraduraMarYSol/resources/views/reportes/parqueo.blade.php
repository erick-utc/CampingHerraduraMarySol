<x-layouts::app :title="__('Reporte: Uso del Parqueo')">
    <div class="p-6">
        <style>
            @media print {
                .no-print {
        ...
        @php
            ...
            $totalReservas = $registros->sum('total_reservas_con_parqueo');
            $totalEspacios = $registros->sum('total_espacios_parqueo');
        @endphp
        ...
        <div class="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
            <div class="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
                <div>
                    ...
                    <flux:heading size="xl">{{ __('Reporte: Uso del Parqueo') }}</flux:heading>
                    <p class="mt-1 text-sm text-zinc-600 dark:text-zinc-300">{{ __('Reporte dinámico del uso de parqueo por período.') }}</p>
                </div>
        ...
        <div class="no-print mt-4 flex flex-wrap items-end justify-between gap-3">
            <form method="GET" class="flex flex-wrap items-end gap-2">
                ...
                <a href="{{ route('reportes.parqueo') }}" class="rounded-lg bg-neutral-600 px-4 py-2 text-sm text-white hover:bg-neutral-700">{{ __('Limpiar') }}</a>
            </form>
            ...
            <table class="w-full text-left divide-y divide-zinc-200 dark:divide-zinc-700">
                <thead class="bg-zinc-50 dark:bg-zinc-800">
                    <tr>
                        ...
                        <th class="px-4 py-2 text-sm">{{ __('Reservas con Parqueo') }}</th>
                        <th class="px-4 py-2 text-sm">{{ __('Total Espacios Parqueo') }}</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                    @forelse($registros as $row)
                        <tr>
                            ...
                            <td class="px-4 py-2 text-sm">{{ $row->total_reservas_con_parqueo }}</td>
                            <td class="px-4 py-2 text-sm">{{ $row->total_espacios_parqueo }}</td>
                        </tr>
                    @empty
                        <tr><td colspan="3" class="px-4 py-4 text-sm text-zinc-500">{{ __('Sin datos') }}</td></tr>
                    @endforelse
                </tbody>
                @if($registros->count() > 0)
                    <tfoot class="bg-zinc-100 dark:bg-zinc-800/80">
                        <tr>
                            <td class="px-4 py-3 text-sm font-semibold">{{ __('Totalización') }}</td>
                            <td class="px-4 py-3 text-sm font-bold">{{ $totalReservas }}</td>
                            <td class="px-4 py-3 text-sm font-bold">{{ $totalEspacios }}</td>
                        </tr>
                    </tfoot>
                @endif
            </table>
        ...
        <div class="mt-4 rounded-lg bg-zinc-50 px-4 py-3 text-sm text-zinc-600 dark:bg-zinc-900 dark:text-zinc-300">
            <p>{{ __('Pie de página del reporte: consolidado del uso de parqueo.') }}</p>
