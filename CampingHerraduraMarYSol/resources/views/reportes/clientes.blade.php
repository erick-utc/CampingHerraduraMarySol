<x-layouts::app :title="__('Reporte: Número de Clientes')">
    <div class="p-6">
        <style>
            @media print {
                .no-print {
        ...
        @php
            ...
            $totalClientes = $registros->sum('total_clientes');
        @endphp
        ...
        <div class="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
            <div class="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
                <div>
                    ...
                    <flux:heading size="xl">{{ __('Reporte: Número de Clientes') }}</flux:heading>
                    <p class="mt-1 text-sm text-zinc-600 dark:text-zinc-300">{{ __('Encabezado del reporte con información dinámica filtrada por período.') }}</p>
                </div>
        ...
        <div class="no-print mt-4 flex flex-wrap items-end justify-between gap-3">
            <form method="GET" class="flex flex-wrap items-end gap-2">
                ...
                <a href="{{ route('reportes.clientes') }}" class="rounded-lg bg-neutral-600 px-4 py-2 text-sm text-white hover:bg-neutral-700">{{ __('Limpiar') }}</a>
            </form>
            ...
            <table class="w-full text-left divide-y divide-zinc-200 dark:divide-zinc-700">
                <thead class="bg-zinc-50 dark:bg-zinc-800">
                    <tr>
                        ...
                        <th class="px-4 py-2 text-sm">{{ __('Inicio') }}</th>
                        <th class="px-4 py-2 text-sm">{{ __('Fin') }}</th>
                        <th class="px-4 py-2 text-sm">{{ __('Total Clientes') }}</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                    @forelse($registros as $row)
                        <tr>
                            <td class="px-4 py-2 text-sm">{{ strtoupper($row->periodo_tipo) }}</td>
                            <td class="px-4 py-2 text-sm">{{ optional($row->periodo_inicio)->format('d/m/Y') }}</td>
                            <td class="px-4 py-2 text-sm">{{ optional($row->periodo_fin)->format('d/m/Y') }}</td>
                            <td class="px-4 py-2 text-sm font-semibold">{{ $row->total_clientes }}</td>
                        </tr>
                    @empty
                ...
                @if($registros->count() > 0)
                    <tfoot class="bg-zinc-100 dark:bg-zinc-800/80">
                        <tr>
                            <td colspan="3" class="px-4 py-3 text-sm font-semibold">{{ __('Totalización') }}</td>
                            <td class="px-4 py-3 text-sm font-bold">{{ $totalClientes }}</td>
                        </tr>
                    </tfoot>
                @endif
            </table>
        ...
        <div class="mt-4 rounded-lg bg-zinc-50 px-4 py-3 text-sm text-zinc-600 dark:bg-zinc-900 dark:text-zinc-300">
            <p>{{ __('Pie de página del reporte: generado automáticamente por el sistema.') }}</p>
