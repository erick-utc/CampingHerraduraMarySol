<x-layouts::app :title="__('Editar Factura')">
    <div class="p-6 max-w-4xl">
        <flux:heading size="xl">{{ __('Editar Factura') }} - {{ $factura->numero_factura }}</flux:heading>

        <form method="POST" action="{{ route('facturas.update', $factura) }}" class="mt-6 space-y-5" id="factura-form">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="reserva_id" id="reserva_id" class="w-full rounded-lg border px-3 py-2" required>
                    ...
                    @foreach($reservas as $reserva)
                        <option value="{{ $reserva->id }}" data-precio-reserva="{{ (float) $reserva->precio }}" @selected(old('reserva_id', $factura->reserva_id) == $reserva->id)>
            ...
            <div>
                ...
                <input type="datetime-local" name="fecha_factura" value="{{ old('fecha_factura', optional($factura->fecha_factura)->format('Y-m-d\\TH:i')) }}" class="w-full rounded-lg border px-3 py-2" required>
            ...
            <div>
                ...
                <select name="porcentaje_impuesto" id="porcentaje_impuesto" class="w-full rounded-lg border px-3 py-2" required>
                    @for($i = 1; $i <= 15; $i++)
                        <option value="{{ $i }}" @selected(old('porcentaje_impuesto', $porcentajeImpuesto) == $i)>{{ $i }}%</option>
                    @endfor
                </select>
                        ...
                        <li class="producto-item flex items-center justify-between gap-3 border-b pb-2 last:border-b-0" data-search="{{ mb_strtolower($producto->marca.' '.$producto->producto.' '.$producto->tamano) }}">
                            <label class="flex items-center gap-2">
                                <input
                                    ...
                                    @checked(in_array($producto->id, old('productos', $selectedProductos)))
                                >
                            ...
                            <div class="flex items-center gap-1">
                                <button
                                    type="button"
                                    class="cantidad-btn rounded border px-2 py-1"
                                ...
                                <input
                                    ...
                                    value="{{ old('cantidades.' . $producto->id, $cantidades[$producto->id] ?? 1) }}"
                                    class="w-20 rounded-lg border px-2 py-1 text-center producto-cantidad"
                                    data-producto-id="{{ $producto->id }}"
                                >
    ...
    <script>
        ...
        document.querySelectorAll('.cantidad-btn').forEach((button) => {
            button.addEventListener('click', function () {
                ...
                const input = document.querySelector('.producto-cantidad[data-producto-id="' + productoId + '"]');
