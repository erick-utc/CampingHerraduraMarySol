<x-layouts::app :title="__('Nueva Reserva')">
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl p-6 max-w-3xl">
        <h1 class="text-2xl font-bold">{{ __('Nueva Reserva') }}</h1>
        ...
        <form action="{{ route('reservas.store') }}" method="POST" class="space-y-4">
                ...
                <select name="usuario_id" id="usuario_id" class="w-full rounded-lg border px-3 py-2" required>
                    ...
                    @foreach($usuarios as $usuario)
                        <option
                            ...
                            @selected(old('usuario_id') == $usuario->id)
                        >
                ...
                <select name="hospedaje_id" id="hospedaje_id" class="w-full rounded-lg border px-3 py-2" required>
                    ...
                    @foreach($hospedajes as $hospedaje)
                        <option
                            ...
                            @selected(old('hospedaje_id', $hospedajeSeleccionado ?? null) == $hospedaje->id)
                        >
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div>
                    ...
                    <input type="datetime-local" name="fecha_entrada" class="w-full rounded-lg border px-3 py-2" value="{{ old('fecha_entrada') }}" required>
                    @error('fecha_entrada')
                        <p class="text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="datetime-local" name="fecha_salida" class="w-full rounded-lg border px-3 py-2" value="{{ old('fecha_salida') }}" required>
                    @error('fecha_salida')
                        <p class="text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                ...
                <div>
                    ...
                    <select name="estado" class="w-full rounded-lg border px-3 py-2" required>
                        <option value="creado" @selected(old('estado') === 'creado')>{{ __('Creado') }}</option>
                        <option value="en espera" @selected(old('estado') === 'en espera')>{{ __('En espera') }}</option>
                        <option value="aprobado" @selected(old('estado') === 'aprobado')>{{ __('Aprobado') }}</option>
                        <option value="cancelado" @selected(old('estado') === 'cancelado')>{{ __('Cancelado') }}</option>
                    </select>
                    @error('estado')
                        <p class="text-sm text-red-600">{{ $message }}</p>
                    @enderror
            ...
            <div>
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="desayuno" value="1" @checked(old('desayuno'))>
